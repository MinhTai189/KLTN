...
const uploadPost = require("../middleware/upload-post");

router.post("/uploads/posts", uploadPost, async (req, res) => {
  if (req.results.length > 0)
    return res.status(200).json({ url: req.results[0].url });
  else if (req.results.length == 0)
    return res
      .status(500)
      .json({ error: { message: "Không thể tải hỉnh ảnh!" } });
});
