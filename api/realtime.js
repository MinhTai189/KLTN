...
module.exports.listen = function socket(server) {
        ...
        async (err, data) => {
          if (err) {
            ...
            if (
              !io.users.some(
                (user) =>
                  JSON.stringify(user._id) === JSON.stringify(data.id) &&
              ...
              .then((res) => {
                if (res)
                  res.forEach((item) => {
                    ...
                    io.to(`${item._id.toString()}_global`).emit(
                      "ononlines-chat",
                      {
    ...
    socket.on("disconnect", () => {
      ...
      if (typeof findUserDisconnect !== "undefined") {
        ...
        groupChat
          ...
          .then((res) => {
            res.forEach((group) => {
              io.in(group._id.toString() + "_global").emit("ononlines-chat", {
                list: listOnline.getUsers(1, -1).list.filter((user) => {
  ...
  io.membersOnChangeLeaveGroup = async (groupId, group, userId) => {
    ...
    findSockets.forEach((findSocket) =>
      io.to(findSocket.socketId).emit("quit-group", groupId.toString())
    );
  };
